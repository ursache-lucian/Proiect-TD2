// CompanyDashboard.jsx — pagina principala pentru companii cu mesagerie

import { useState, useEffect } from "react";
import JobForm from "../components/JobForm";

function CompanyDashboard({ user, onLogout, setPagina, setConversatieInitiala }) {
  const [joburi, setJoburi] = useState([]);
  const [loading, setLoading] = useState(true);
  const [eroare, setEroare] = useState(null);
  const [mesajSucces, setMesajSucces] = useState(null);
  const [jobSelectat, setJobSelectat] = useState(null);
  ...
  // Deschidem inbox-ul cu conversatia pre-selectata catre studentul respectiv
  function handleTrimitemesaj(student) {
    setConversatieInitiala({
    ...
    <div style={{ minHeight: "100vh", background: "#f5f3ff" }}>
      <div
        style={{
          ...
          color: "white", padding: "48px 0 32px",
        }}
      >
        <div className="container">
          <h1 className="fw-bold mb-1" style={{ fontSize: "2rem" }}>🏢 Dashboard Companie</h1>
      ...
      <div className="container py-5">
        ...
        {!loading && joburi.length > 0 && (
          <div className="d-flex flex-column gap-3">
            {joburi.map((job) => (
              <div key={job.id} className="card border-0 shadow-sm" style={{ borderRadius: "12px", overflow: "hidden" }}>
                  ...
                  <div className="d-flex justify-content-between align-items-start">
                    <div className="flex-fill me-3">
                      ...
                      <div className="d-flex gap-2 flex-wrap mb-2">
                        <span className="badge" style={{ background: "#ede9fe", color: "#5b21b6" }}>📍 {job.location}</span>
                        <span className="badge" style={{ background: "#dbeafe", color: "#1d4ed8" }}>💼 {job.job_type}</span>
                        <span className="badge" style={{ background: "#f3f4f6", color: "#6b7280" }}>🕒 {formatData(job.created_at)}</span>
                      </div>
                    ...
                    <div className="d-flex flex-column gap-2">
                      <button
                        ...
                        style={{ border: "1.5px solid #ede9fe", borderRadius: "8px", color: "#4f46e5", background: "#f5f3ff", fontWeight: 600, whiteSpace: "nowrap" }}
                      >
                        👥 Vezi aplicanți
                      </button>
                      <button
                        ...
                        className="btn btn-sm"
                        style={{ border: "1.5px solid #fee2e2", borderRadius: "8px", color: "#991b1b", background: "#fff5f5", fontWeight: 600, whiteSpace: "nowrap" }}
                      >
                        🗑 Șterge
                      </button>
                    </div>
      ...
      {jobSelectat && (
        <>
          <div onClick={inchideModal} style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", zIndex: 200 }} />
          <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", background: "white", borderRadius: "16px", width: "90%", maxWidth: "600px", maxHeight: "85vh", overflowY: "auto", zIndex: 201, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ height: "4px", background: "linear-gradient(90deg, #4f46e5, #7c3aed)", borderRadius: "16px 16px 0 0" }} />
            <div className="p-4">
              <div className="d-flex justify-content-between align-items-start mb-4">
                <div>
                  <h4 className="fw-bold mb-1" style={{ color: "#1e1b4b" }}>👥 Aplicanți</h4>
                  <p className="text-muted mb-0" style={{ fontSize: "0.9rem" }}>{jobSelectat.title}</p>
                </div>
                <button onClick={inchideModal} className="btn btn-sm" style={{ border: "1.5px solid #e5e7eb", borderRadius: "8px", color: "#6b7280", fontWeight: 700 }}>✕</button>
              </div>
              ...
              {!loadingAplicanti && aplicanti.length > 0 && (
                <div className="d-flex flex-column gap-3">
                  ...
                  {aplicanti.map((student) => (
                    <div key={student.user_id} className="card border-0" style={{ background: "#f9f8ff", borderRadius: "12px" }}>
                      <div className="card-body p-3">
                        <div className="d-flex align-items-center justify-content-between mb-2">
                          <div className="d-flex align-items-center gap-2">
                            <div
                              className="d-flex align-items-center justify-content-center fw-bold flex-shrink-0"
                              style={{ width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg, #4f46e5, #7c3aed)", color: "white", fontSize: "1.1rem" }}
                            >
                          ...
                          {/* Buton trimite mesaj */}
                          <button
                            onClick={() => handleTrimitemesaj(student)}
                            className="btn btn-sm"
                            style={{ border: "1.5px solid #ede9fe", borderRadius: "8px", color: "#4f46e5", background: "#f5f3ff", fontWeight: 600, whiteSpace: "nowrap" }}
                          >
                            💬 Mesaj
                        ...
                        {student.description && (
                          <p className="mb-2" style={{ fontSize: "0.88rem", color: "#4b5563" }}>{student.description}</p>
                        )}
